...
parameters {
  ...
  vector[2] mu_p;                   // group-level parameter means
  vector<lower=0>[2] sigma_p;       // group-level parameter sigmas (must be positive)
...
transformed parameters {
  ...
  for (p in 1:nPpts) {
    alpha[p] = Phi_approx(mu_p[1] + sigma_p[1] * alpha_raw[p]);      // implies alpha ~ normal(mu_alpha, sigma_alpha), constrained to be in range [0,1]
    beta[p]   = Phi_approx(mu_p[2] + sigma_p[2] * beta_raw[p])*20;           // implies beta ~ normal(mu_beta, sigma_beta), constrained to be in range [0,10]
  }
}
  ...
  for (p in 1:nPpts) {
    ...
    for (t in 1:nT_ppts[p]) {
      ...
      if ( valence[p,t] == 0 ) {
        ...
        q_neg[t+1, choice[p,t]] = q_neg[t, choice[p,t]] + alpha[p]*(outcome[p,t] - q_neg[t,choice[p,t]]);
      } else {
        // choice probability is a function of q values
        choice[p,t] ~ categorical_logit(beta[p] * q_pos[t]);
        // update value of chosen option
        q_pos[t+1, choice[p,t]] = q_pos[t, choice[p,t]] + alpha[p]*(outcome[p,t] - q_pos[t,choice[p,t]]);
      }
  ...
  for (p in 1:nPpts) {
    ...
    for (t in 1:nT_ppts[p]) {
      ...
      if ( valence[p,t] == 0 ) {
        ...
        //update value of chosen option
        q2_neg[t+1, y_rep[p,t]] = q2_neg[t, y_rep[p,t]] + alpha[p]*(outcome_sim[p,t] - q2_neg[t, y_rep[p,t]]);
        //q2_neg[t+1, y_rep[p,t]] = q2_neg[t, y_rep[p,t]] + alpha[p]*(outcome[p,t] - q2_neg[t, y_rep[p,t]]);
        ...
        //update value of chosen option
        q2_pos[t+1, y_rep[p,t]] = q2_pos[t, y_rep[p,t]] + alpha[p]*(outcome_sim[p,t] - q2_pos[t, y_rep[p,t]]);
        //q2_pos[t+1, y_rep[p,t]] = q2_pos[t, y_rep[p,t]] + alpha[p]*(outcome[p,t] - q2_pos[t, y_rep[p,t]]);
      }
