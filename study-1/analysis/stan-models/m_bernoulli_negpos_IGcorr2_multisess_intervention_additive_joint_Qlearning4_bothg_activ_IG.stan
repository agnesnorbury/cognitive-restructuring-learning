...
parameters {
  ...
  real beta_both_internal; 
  real beta_both_global; 
  real beta_activ_internal; 
  real beta_activ_global; 
...
transformed parameters {
  ...
  for ( p in 1:nPpts ) {
    ...
    beta[p]   = Phi_approx(mu_p[3] + sigma_p[3] * beta_raw[p])*20;           // implies beta ~ normal(mu_beta, sigma_beta), constrained to be in range [0,10]
    q0_1_IG_neg[p] = (mu_p[4] + sigma_p[4] * q0_1_IG_neg_raw[p]);      // implies alpha ~ normal(mu_alpha, sigma_alpha)
    q0_1_IG_pos[p] = (mu_p[5] + sigma_p[5] * q0_1_IG_pos_raw[p]);      // implies alpha ~ normal(mu_alpha, sigma_alpha)]
    q0_23_IG_neg[p] = (mu_p[6] + sigma_p[6] * q0_23_IG_neg_raw[p]);      // implies alpha ~ normal(mu_alpha, sigma_alpha)
    q0_23_IG_pos[p] = (mu_p[7] + sigma_p[7] * q0_23_IG_pos_raw[p]);      // implies alpha ~ normal(mu_alpha, sigma_alpha)]
    ...
    if ( condition[p] == 1 ) {
      // for active intervention participants
      theta_internal_pos[p,2] = mu_internal_theta_pos[2] + pars_tilde_pos[3,p] + theta_int_internal_pos + beta_both_internal*alpha_pos[p] + beta_activ_internal*alpha_pos[p];
      theta_global_pos[p,2]   = mu_global_theta_pos[2]   + pars_tilde_pos[4,p] + theta_int_global_pos + beta_both_global*alpha_pos[p] + beta_activ_global*alpha_pos[p];
    } else {
      // for control intervention participants
      theta_internal_pos[p,2] = mu_internal_theta_pos[2] + pars_tilde_pos[3,p] + beta_both_internal*alpha_pos[p];
      theta_global_pos[p,2]   = mu_global_theta_pos[2]   + pars_tilde_pos[4,p] + beta_both_global*alpha_pos[p];
    }
  }
}
...
model {
  ...
  mu_p ~ normal(0,1);
  sigma_p ~ cauchy(0,1);
  // delta_q0_neg ~ normal(0,1);
  // delta_q0_pos ~ normal(0,1);
