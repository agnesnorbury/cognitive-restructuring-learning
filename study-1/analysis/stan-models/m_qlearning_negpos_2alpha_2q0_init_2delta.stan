...
parameters {
  ...
  vector[3] mu_p;                   // group-level parameter means
  vector<lower=0>[3] sigma_p;       // group-level parameter sigmas (must be positive)
  ...
  // group-level q0 values for each valence (same across blocks)
  real q0_IG_neg;
  real q0_IG_pos;
  real delta_q0_IG_neg;
  real delta_q0_IG_pos;
}
...
model {
  ...
  // priors on group-level starting values
  q0_IG_neg ~ normal(0.5,1);
  q0_IG_pos ~ normal(0.5,1);
  delta_q0_IG_neg ~ normal(0,1);
  delta_q0_IG_pos ~ normal(0,1);
  ...
  for (p in 1:nPpts) {
    ...
    for (t in 1:nT_ppts[p]) {
      ...
      if ( t == 1 ) {
        q_neg[t] = [0.5, q0_IG_neg]';                  
        q_pos[t] = [0.5, q0_IG_pos]';                  
      } else if ( t == 11 ) {
        q_neg[t] = [0.5, q0_IG_neg + delta_q0_IG_neg]';                  
        q_pos[t] = [0.5, q0_IG_pos + delta_q0_IG_pos]'; 
      } else if ( t == 21 ) {
        q_neg[t] = [0.5, q0_IG_neg + 2*delta_q0_IG_neg]';                  
        q_pos[t] = [0.5, q0_IG_pos + 2*delta_q0_IG_pos]'; 
      }
...
generated quantities {
  ...
  for (p in 1:nPpts) {
    ...
    for (t in 1:nT_ppts[p]) {
      ...
      if ( t == 1 ) {
        q2_neg[t] = [0.5, q0_IG_neg]';                  
        q2_pos[t] = [0.5, q0_IG_pos]';                  
      } else if ( t == 11 ) {
        q2_neg[t] = [0.5, q0_IG_neg + delta_q0_IG_neg]';                  
        q2_pos[t] = [0.5, q0_IG_pos + delta_q0_IG_pos]'; 
      } else if ( t == 21 ) {
        q2_neg[t] = [0.5, q0_IG_neg + 2*delta_q0_IG_neg]';                  
        q2_pos[t] = [0.5, q0_IG_pos + 2*delta_q0_IG_pos]'; 
      }
