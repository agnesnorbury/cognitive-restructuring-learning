...
parameters {
  ...
  vector[5] mu_p;                   // group-level parameter means
  vector<lower=0>[5] sigma_p;       // group-level parameter sigmas (must be positive)
  real delta_q0_neg;
  real delta_q0_pos;
  ...
  vector[nPpts] q0_IG_neg_raw;
  vector[nPpts] q0_IG_pos_raw;
}
...
transformed parameters {
  ...
  vector[nPpts] beta;
  vector[nPpts] q0_IG_neg; 
  vector[nPpts] q0_IG_pos;
  ...
  for (p in 1:nPpts) {
    ...
    beta[p]   = Phi_approx(mu_p[3] + sigma_p[3] * beta_raw[p])*20;           // implies beta ~ normal(mu_beta, sigma_beta), constrained to be in range [0,15]
    q0_IG_neg[p] = (mu_p[4] + sigma_p[4] * q0_IG_neg_raw[p]);      // implies alpha ~ normal(mu_alpha, sigma_alpha)
    q0_IG_pos[p] = (mu_p[5] + sigma_p[5] * q0_IG_pos_raw[p]);      // implies alpha ~ normal(mu_alpha, sigma_alpha)]
  }
}
...
model {
  ...
  beta_raw ~ normal(0,1);  
  q0_IG_neg_raw ~ normal(0,1);
  q0_IG_pos_raw ~ normal(0,1);
  ...
  for (p in 1:nPpts) {
    ...
    for (t in 1:nT_ppts[p]) {
      ...
      if ( t == 1 ) {
        q_neg[t] = [0.5, q0_IG_neg[p]]';                  
        q_pos[t] = [0.5, q0_IG_pos[p]]';                  
      } else if ( t == 11 ) {
        q_neg[t] = [0.5, q0_IG_neg[p] + delta_q0_neg]';                  
        q_pos[t] = [0.5, q0_IG_pos[p] + delta_q0_pos]';  
      } else if (t == 21 ) {
        q_neg[t] = [0.5, q0_IG_neg[p] + 2*delta_q0_neg]';                  
        q_pos[t] = [0.5, q0_IG_pos[p] + 2*delta_q0_pos]'; 
      }
...
generated quantities {
  ...
  for (p in 1:nPpts) {
    ...
    for (t in 1:nT_ppts[p]) {
      ...
      if ( t == 1 ) {
        q2_neg[t] = [0.5, q0_IG_neg[p]]';                  
        q2_pos[t] = [0.5, q0_IG_pos[p]]';                  
      } else if ( t == 11 ) {
        q2_neg[t] = [0.5, q0_IG_neg[p] + delta_q0_neg]';                  
        q2_pos[t] = [0.5, q0_IG_pos[p] + delta_q0_pos]';  
      } else if (t == 21 ) {
        q2_neg[t] = [0.5, q0_IG_neg[p] + 2*delta_q0_neg]';                  
        q2_pos[t] = [0.5, q0_IG_pos[p] + 2*delta_q0_pos]'; 
      }
