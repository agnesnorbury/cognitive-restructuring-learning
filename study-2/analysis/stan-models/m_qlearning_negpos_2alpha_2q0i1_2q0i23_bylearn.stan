...
data {
  ...
  int learn_cond[nPpts];           // actual number of trials per participant
  int nChoices;
  int blockLength;
...
transformed parameters {
  ...
  for (p in 1:nPpts) {
    ...
    if ( learn_cond[p] == 1 ) {
      ...
      beta[p]   = Phi_approx(mu_p[13] + sigma_p[13] * beta_raw[p])*20;           // implies beta ~ normal(mu_beta, sigma_beta), constrained to be in range [0,15]
  }
}
