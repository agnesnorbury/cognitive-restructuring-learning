...
parameters {
  ...
  // group-level effects of learning task at t2 (group level)
...
transformed parameters {
  ...
  for ( p in 1:nPpts ) {
    ...
    if ( int_condition[p] == 1  && learn_condition[p] == 1 ) {
      // for active intervention participants who did causal learning
      ...
      // for control intervention participants who did causal learning
      ...
      // for active intervention participants who did control learning
    ...
    if ( int_condition[p] == 1 && learn_condition[p] == 1 ) {
      ...
      theta_internal_pos[p,2] = mu_internal_theta_pos[2] + pars_tilde_pos[3,p] + theta_int_internal_pos + theta_learn_internal_pos;
      theta_global_pos[p,2]   = mu_global_theta_pos[2]   + pars_tilde_pos[4,p] + theta_int_global_pos + theta_learn_global_pos;
    } else if ( int_condition[p] == 0  && learn_condition[p] == 1 ) {
      // for control intervention participants who did causal learning
      theta_internal_pos[p,2] = mu_internal_theta_pos[2] + pars_tilde_pos[3,p] + theta_learn_internal_pos;
      theta_global_pos[p,2]   = mu_global_theta_pos[2]   + pars_tilde_pos[4,p] + theta_learn_global_pos;
    } else {
      // for active intervention participants who did control learning
...
model {
  ...
  // priors on group-level effects of learning task on theta values at t2
...
generated quantities {
  ...
  for (p in 1:nPpts) {
    ...
    // replicated data unvectorised as can't get to work otherwise :-(
